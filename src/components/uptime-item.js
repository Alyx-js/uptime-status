...
const UptimeItem = (props) => {
  ...
  const total = useMemo(() => {
    return monitor.total.times
      ? `Recent ${CountDays} Days, ${monitor.total.times} Times，For ${formatDuration(monitor.total.duration)}，Average: ${monitor.average}%`
      : `Recent ${CountDays}, Uptime: ${monitor.average}%`;
  }, [CountDays, monitor]);
